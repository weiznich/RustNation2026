...
pub(crate) fn insert_test_data(conn: &mut SqliteConnection) -> QueryResult<()> {
    conn.transaction(|conn| {
        ...
        let competition_id = diesel::insert_into(competitions::table)
            .values((
                competitions::name.eq("London City Marathon"),
                competitions::description.eq("An example competition for RustNation 2026 in London"),
                competitions::date
                    .eq(Date::from_calendar_date(2026, time::Month::February, 18).expect("We know that this is a valid date")),
                competitions::location.eq("London"),
        ...
        let inserted_starts = diesel::insert_into(starts::table)
            .values(vec![
                (
                    starts::name.eq("400m"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 10:00:00)),
                    starts::race_id.eq(race_map["400m"]),
                ),
                (
                    starts::name.eq("800m"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 10:10:00)),
                    starts::race_id.eq(race_map["800m"]),
                ),
                (
                    starts::name.eq("1200m"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 10:20:00)),
                    starts::race_id.eq(race_map["1200m"]),
                ),
                (
                    starts::name.eq("5,5km Nordic Walking"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 10:25:00)),
                    starts::race_id.eq(race_map["5,5km Nordic Walking"]),
                ),
                (
                    starts::name.eq("2,5km"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 10:30:00)),
                    starts::race_id.eq(race_map["2,5km"]),
                ),
                (
                    starts::name.eq("11km"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 10:50:00)),
                    starts::race_id.eq(race_map["11km"]),
                ),
                (
                    starts::name.eq("5,5km (Femal)"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 11:00:00)),
                    starts::race_id.eq(race_map["5,5km"]),
                ),
                (
                    starts::name.eq("5,5km (Male)"),
                    starts::time.eq(time::macros::datetime!(2026-02-18 11:10:00)),
                    starts::race_id.eq(race_map["5,5km"]),
                ),
            ])
            .execute(conn)?;
        ...
        diesel::insert_into(participants::table)
            .values([
                (
                    ...
                    participants::club.eq("RustNation Running Club"),
