...
async fn check_registration() {
    ...
    state
        .with_connection(move |conn| {
            ...
            assert_eq!(club, Some("RustNation Running Club".into()));
            assert_eq!(conset_agb, true);
            assert_eq!(birth_year, 1996);
            let (cat_name, race_name) = categories::table
                .find(category_id)
                .inner_join(starts::table.inner_join(races::table))
                .select((categories::label, races::name))
                .get_result::<(String, String)>(conn)?;
            assert_eq!(cat_name, "M 31");
            assert_eq!(race_name, "11km");
            Ok(())
        })
        .await
        .unwrap();
